...
import bg from "./assets/Music_Abstract_dn.webp";
...
function App() {
  ...
  const { login, setLogin } = useContext(UserContext); // Access login state from UserContext
  ...
  const fetchMusicData = async () => {
    ...
    try {
      const response = await fetch(
        `https://v1.nocodeapi.com/ishubham/spotify/UaajykXVhuLyTyWZ/search?q=${keyword === "" ? "trending" : keyword}&type=track`,
        {
          headers: {
